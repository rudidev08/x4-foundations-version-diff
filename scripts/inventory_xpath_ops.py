...
- attributes present: sel, if, position, silent, sel_has_attr_terminator
- position values seen
- XPath axis tokens: //, /, predicates, not(), @attribute
Output a human-readable table to tests/fixtures/_entity_diff_golden/xpath_inventory.text.
"""
...
ROOT = Path('x4-data')
OUT = Path('tests/fixtures/_entity_diff_golden/xpath_inventory.text')
    ...
    for ver in sorted(ROOT.glob('*')):
        ...
        for xml in ver.glob('extensions/*/**/*.xml'):
            ...
            for operation in root:
                ...
                if selector:
                    sel_patterns[_shape(selector)] += 1
                gate = operation.get('if')
                if gate:
                    if_patterns[_shape(gate)] += 1
                if operation.get('position'):
                    pos_values[operation.get('position')] += 1
                if operation.get('silent'):
                    silent_values[operation.get('silent')] += 1
