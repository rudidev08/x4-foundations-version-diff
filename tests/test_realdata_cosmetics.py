"""Real-data tests for the `cosmetics` rule.

Tier A — smoke: runs against every configured pair and verifies a list.
Tier B — snapshot: canonical 8.00H4 → 9.00B6 pair, sorted one-line-per-output
digest snapshot. Regenerate via
  `X4_REGEN_SNAPSHOT=cosmetics python3 -m unittest tests.test_realdata_cosmetics`.
"""
import os
import sys
import unittest
...
SNAP = HERE / 'snapshots' / 'cosmetics_8.00H4_9.00B6.txt'


def _run(pair):
    cache.clear()
    reset_index()
    return cosmetics.run(CORPUS / pair[0], CORPUS / pair[1])
...
class TierBBaselineTest(unittest.TestCase):
    def test_snapshot_matches(self):
        ...
        actual = '\n'.join(lines) + '\n'
        if not SNAP.exists() or os.environ.get('X4_REGEN_SNAPSHOT') == 'cosmetics':
        ...
        self.assertEqual(
            actual, expected,
            'Cosmetics snapshot drift. '
            'Regen: X4_REGEN_SNAPSHOT=cosmetics python3 -m unittest '
            'tests.test_realdata_cosmetics',
        )
