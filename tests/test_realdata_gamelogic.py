"""Real-data tests for the `gamelogic` rule.
...
digest snapshot. Regenerate via
  `X4_REGEN_SNAPSHOT=gamelogic python3 -m unittest tests.test_realdata_gamelogic`.
"""
import os
import sys
import unittest
...
SNAP = HERE / 'snapshots' / 'gamelogic_8.00H4_9.00B6.txt'


def _run(pair):
    cache.clear()
    reset_index()
    return gamelogic.run(CORPUS / pair[0], CORPUS / pair[1])
...
class TierBBaselineTest(unittest.TestCase):
    def test_snapshot_matches(self):
        ...
        actual = '\n'.join(lines) + '\n'
        if not SNAP.exists() or os.environ.get('X4_REGEN_SNAPSHOT') == 'gamelogic':
        ...
        self.assertEqual(
            actual, expected,
            'Gamelogic snapshot drift. '
            'Regen: X4_REGEN_SNAPSHOT=gamelogic python3 -m unittest tests.test_realdata_gamelogic',
        )
