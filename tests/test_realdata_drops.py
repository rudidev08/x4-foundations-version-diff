"""Real-data tests for the `drops` rule.
...
`X4_REGEN_SNAPSHOT=drops python3 -m unittest tests.test_realdata_drops`.
"""
import os
import sys
import unittest
...
SNAP = HERE / 'snapshots' / 'drops_8.00H4_9.00B6.txt'


def _run(pair):
    cache.clear()
    reset_index()
    return drops.run(CORPUS / pair[0], CORPUS / pair[1])
...
class TierBBaselineTest(unittest.TestCase):
    def test_snapshot_matches(self):
        ...
        actual = '\n'.join(lines) + '\n'
        if not SNAP.exists() or os.environ.get('X4_REGEN_SNAPSHOT') == 'drops':
        ...
        self.assertEqual(
            actual, expected,
            'Drops snapshot drift. '
            'Regen: X4_REGEN_SNAPSHOT=drops python3 -m unittest tests.test_realdata_drops',
        )
