"""Real-data Tier A/B tests for the sectors rule.

Tier A: rule runs without raising across every configured version pair.
Tier B: snapshot of (entity_key, kind, subsource, sha256(text)) on the
        canonical 8.00H4 → 9.00B6 pair. Regenerate with
        `X4_REGEN_SNAPSHOT=sectors python3 -m unittest tests.test_realdata_sectors`.
"""
import os
import sys
import unittest
...
SNAP = HERE / 'snapshots' / 'sectors_8.00H4_9.00B6.txt'
...
class TierBBaselineTest(unittest.TestCase):
    def test_no_unreviewed_incompletes(self):
        ...
        Warnings surface in the snapshot but don't block; they're informational.
        """
        ...
        self.assertEqual(
            ...
            f'unreviewed sectors INCOMPLETEs: {incomplete_unreviewed}',
        )
    ...
    def test_snapshot_matches(self):
        ...
        actual = '\n'.join(lines) + '\n'
        if not SNAP.exists() or os.environ.get('X4_REGEN_SNAPSHOT') == 'sectors':
        ...
        self.assertEqual(
            actual, expected,
            'Sectors snapshot drift. '
            'Regen: X4_REGEN_SNAPSHOT=sectors python3 -m unittest '
            'tests.test_realdata_sectors',
        )
