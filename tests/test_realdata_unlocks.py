"""Real-data tests for the `unlocks` rule.
...
digest snapshot. Regenerate via
  `X4_REGEN_SNAPSHOT=unlocks python3 -m unittest tests.test_realdata_unlocks`.
"""
import os
import sys
import unittest
...
SNAP = HERE / 'snapshots' / 'unlocks_8.00H4_9.00B6.txt'


def _run(pair):
    cache.clear()
    reset_index()
    return unlocks.run(CORPUS / pair[0], CORPUS / pair[1])
...
class TierBBaselineTest(unittest.TestCase):
    def test_snapshot_matches(self):
        ...
        actual = '\n'.join(lines) + '\n'
        if not SNAP.exists() or os.environ.get('X4_REGEN_SNAPSHOT') == 'unlocks':
        ...
        self.assertEqual(
            actual, expected,
            'Unlocks snapshot drift. '
            'Regen: X4_REGEN_SNAPSHOT=unlocks python3 -m unittest tests.test_realdata_unlocks',
        )
