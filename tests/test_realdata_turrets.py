"""Real-data Tier A/B tests for the turrets rule.
...
Tier B: snapshot of (entity_key, kind, subsource, sha256(text)) on the canonical
        ...
        `X4_REGEN_SNAPSHOT=turrets python3 -m unittest tests.test_realdata_turrets`.
"""
import os
import sys
import unittest
...
SNAP = HERE / 'snapshots' / 'turrets_8.00H4_9.00B6.txt'
...
BASELINE = {
    'pair': CANONICAL_PAIR,
    'sentinels': [
        # A turret known to be in real 9.00B6. The rule emits rows only when
        # the ware or its macros diff; the smoke-sentinel just asserts the
        # rule produces *some* output for the canonical pair.
    ],
}
...
class TierBBaselineTest(unittest.TestCase):
    def test_no_unreviewed_incompletes(self):
        ...
        self.assertEqual(
            ...
            f'unreviewed turrets INCOMPLETEs: {incomplete_unreviewed}',
        )
    ...
    def test_snapshot_matches(self):
        ...
        actual = '\n'.join(lines) + '\n'
        if not SNAP.exists() or os.environ.get('X4_REGEN_SNAPSHOT') == 'turrets':
        ...
        self.assertEqual(
            actual, expected,
            'Turrets snapshot drift. '
            'Regen: X4_REGEN_SNAPSHOT=turrets python3 -m unittest tests.test_realdata_turrets',
        )
