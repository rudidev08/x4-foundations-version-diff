...
def resolve(info: MacroInfo) -> MacroLabel | None:
    ...
    if not path.startswith("assets/props/engines/"):
        return None
    if not is_macro_source_path(info.source_path):
        return None
    if not info.macro_name.startswith("engine_"):
        return None
    if info.macro_class != "engine":
        return None
    return make_macro_label("ware", "engine", info.macro_name)
