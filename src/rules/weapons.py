...
def run(old_root: Path, new_root: Path,
    ...
    outputs.extend(_emit_bullet_fanout(
        old_root, new_root, ware_report, locale_old, locale_new,
    ))
...
def _emit_added(new_root: Path, record, locale_new: Locale) -> list[RuleOutput]:
    info = _resolve_macro_info(new_root, record, side='new')
    name = _display_name(record, info, None, locale_new, locale_new)
    classifications = _classify(record.key, record.element, info)
...
def _emit_removed(old_root: Path, record, locale_old: Locale) -> list[RuleOutput]:
    info = _resolve_macro_info(old_root, record, side='old')
    name = _display_name(record, info, None, locale_old, locale_old)
    classifications = _classify(record.key, record.element, info)
...
def _emit_modified(old_root: Path, new_root: Path, record,
    ...
    old_info = _resolve_macro_info(old_root, record, side='old')
    new_info = _resolve_macro_info(new_root, record, side='new')
    name = _display_name(record, new_info, old_info, locale_new, locale_old)
    classifications = _classify(
        record.key, record.new, new_info if new_info.element is not None else old_info)
    ...
    if old_info.element is not None and new_info.element is not None:
        for label, (old_value, new_value) in diff_attrs(old_info.element,
    ...
    return [RuleOutput(tag=TAG, text=text, extras={
        ...
        'sources': record.new_sources,
        'ref_sources': record.new_ref_sources,
    })]


def _build_bullet_reverse_indices(
        effective_root: Optional[ElementTree.Element]) -> dict[str, list[str]]:
...
def _emit_bullet_fanout(old_root: Path, new_root: Path, ware_report,
    ...
    def _populate(ware_root: Path, macroref_to_wares: dict[str, list[str]],
        ...
        for macro_ref, wids in macroref_to_wares.items():
            ...
            if macro_path is None:
                continue
            macro_elem = load_macro(macro_path)
    ...
    for record in ware_report.modified:
        ware_records_by_key[record.key] = ('modified', record)
    for record in ware_report.added:
        ware_records_by_key.setdefault(record.key, ('added', record))
    for record in ware_report.removed:
        ware_records_by_key.setdefault(record.key, ('removed', record))
    ...
    for bullet_ref in shared_bullet_refs:
        ...
        if old_bullet_path is None or new_bullet_path is None:
            continue
        old_bullet = load_macro(old_bullet_path)
        new_bullet = load_macro(new_bullet_path)
        ...
        for wid in impacted_wares:
            row = _emit_bullet_row(
                ...
                old_root, new_root, locale_old, locale_new,
            )
            if row is not None:
                outputs.append(row)
    return outputs
...
def _emit_bullet_row(wid: str, bullet_ref: str, deltas: dict,
                     ...
                     locale_old: Locale, locale_new: Locale) -> Optional[RuleOutput]:
    kind_rec = ware_records_by_key.get(wid)
    if kind_rec is None:
        return None
    kind, record = kind_rec
    if kind == 'modified':
        old_sources = record.old_sources
        new_sources = record.new_sources
        new_info = _resolve_macro_info(new_root, record, side='new')
        ...
        new_sources = record.sources
        new_info = _resolve_macro_info(new_root, record, side='new')
        name = _display_name(record, new_info, None, locale_new, locale_new)
        classifications = _classify(record.key, record.element, new_info)
    else:  # removed
        old_sources = record.sources
        new_sources = None
        old_info = _resolve_macro_info(old_root, record, side='old')
        name = _display_name(record, old_info, None, locale_old, locale_old)
        classifications = _classify(record.key, record.element, old_info)

    parts = [f'{label} {old_value}→{new_value}' for label, (old_value, new_value) in deltas.items()]
    sources_label = render_sources(old_sources, new_sources)
    src_label = f' {sources_label}' if sources_label else ''
...
def _resolve_macro_info(root: Path, record, *, side: str) -> _MacroInfo:
    ...
    is passed (both branches return the same record.element).
    """
    if side == 'old':
        ref_sources = getattr(record, 'old_ref_sources', None) or \
                      getattr(record, 'ref_sources', None) or {}
        element = record.old if hasattr(record, 'old') else record.element
    else:
        ref_sources = getattr(record, 'new_ref_sources', None) or \
                      getattr(record, 'ref_sources', None) or {}
        element = record.new if hasattr(record, 'new') else record.element
    ...
    element_loaded = load_macro(path)
...
def _display_name(record, primary: _MacroInfo, fallback: Optional[_MacroInfo],
                  loc_primary: Locale, loc_fallback: Locale) -> str:
    """Resolve display name from macro's identification/@name, fall back to
    the other side's macro, then to the wares.xml @name attribute, then to the
    ware id.
    """
    ...
    for info, locale in tried:
        m = info.element if info is not None else None
        if m is None:
            continue
        ident = m.find('properties/identification')
        if ident is not None:
            val = resolve_attr_ref(ident, locale, attribute='name',
                                    fallback=m.get('name'))
            if val:
                return val
    # Wares.xml also carries @name on the ware — use it if present.
    element = getattr(record, 'new', None) or getattr(record, 'old', None) \
              or getattr(record, 'element', None)
    if element is not None:
        val = resolve_attr_ref(element, loc_primary, attribute='name',
                                fallback=record.key)
        if val:
            return val
    return record.key
...
def _classify(ware_id: str, ware_element: Optional[ElementTree.Element],
