...
from src.lib.entity_diff import apply_patch, diff_library
...
def _materialize_aiscript(root: Path, filename: str,
                          ...
                          ) -> tuple[Optional[ElementTree.Element], set[str]]:
    ...
    if core_path.is_file():
        try:
            eff = ElementTree.parse(core_path).getroot()
            sources.add('core')
    ...
    for dlc_path in sorted(root.glob(f'extensions/*/aiscripts/{filename}')):
        ...
        short = source_of(rel)
        try:
            dlc_root = ElementTree.parse(dlc_path).getroot()
        except ElementTree.ParseError as e:
            report.failures.append((
                f'parse error {rel}',
                {'reason': 'parse_error', 'detail': str(e),
...
def _emit_aiscripts(old_root: Path, new_root: Path,
    ...
    for filename in all_names:
        ...
        # `[tag] name (cls) [sources_label]: body` shape every other rule uses.
        ...
        text = format_row(TAG, display, classifications, sources_label, [summary])
...
def _aiscript_display_name(new_eff: Optional[ElementTree.Element],
                           old_eff: Optional[ElementTree.Element],
                           stem: str) -> str:
...
def _index_behaviours(tree_root: Optional[ElementTree.Element]
                      ) -> dict[tuple, ElementTree.Element]:
    ...
    pmap: dict[int, ElementTree.Element] = {id(c): p for p in tree_root.iter()
                                   for c in p}
...
def _emit_behaviour_added(composite: tuple, element: ElementTree.Element,
                          report) -> RuleOutput:
    set_name, parent_tag, behaviour_name = composite
    has_bad = _check_behaviour_children(element, composite, report)
    classifications = ['behaviour', set_name, parent_tag]
    attributes = _collect_all_attrs(element, exclude=('name',))
    parts = ['NEW']
    parts.extend(f'{k}={v}' for k, v in sorted(attributes.items()))
    sources_label = render_sources(None, ['core'])
    text = format_row(TAG, behaviour_name, classifications, sources_label, parts)
    extras = {
...
def _emit_behaviour_removed(composite: tuple, element: ElementTree.Element,
                            report) -> RuleOutput:
    set_name, parent_tag, behaviour_name = composite
    _check_behaviour_children(element, composite, report)
    classifications = ['behaviour', set_name, parent_tag]
    sources_label = render_sources(['core'], None)
    text = format_row(TAG, behaviour_name, classifications, sources_label, ['REMOVED'])
    extras = {
        'entity_key': ('behaviour', composite),
        'kind': 'removed',
        'subsource': 'behaviour',
...
def _emit_behaviour_modified(composite: tuple, old_el: ElementTree.Element,
                             new_el: ElementTree.Element,
    ...
    changes.extend(diff_attr_map(old_attrs, new_attrs))
    ...
    text = format_row(TAG, behaviour_name, classifications, sources_label, changes)
    extras = {
        'entity_key': ('behaviour', composite),
        'kind': 'modified',
        'subsource': 'behaviour',
...
def _check_behaviour_children(element: ElementTree.Element, composite: tuple,
                              report) -> bool:
    ...
    """
    bad = {c.tag for c in element if c.tag not in _BEHAVIOUR_CHILD_TAGS}
...
def _diff_behaviour_children(old_el: ElementTree.Element,
                             new_el: ElementTree.Element) -> list[str]:
    """Diff whitelisted child collections under <behaviour>.

    - `<param>` children keyed by `@name`.
    - `<precondition>` / `<script>` singletons — attributes diffed directly.
    ...
    for tag in ('precondition', 'script'):
        ...
        if o is None and n is None:
            continue
        if o is None:
            attributes = ', '.join(f'{k}={v}' for k, v in sorted(n.attrib.items()))
            out.append(f'{tag} added' + (f' ({attributes})' if attributes else ''))
            continue
        if n is None:
...
def _index_properties(tree_root: Optional[ElementTree.Element]
                      ) -> dict[tuple, ElementTree.Element]:
...
def _emit_scriptproperty_added(composite: tuple, element: ElementTree.Element,
                               report) -> RuleOutput:
    dt_name, pname = composite
    has_bad = _check_scriptproperty_children(element, composite, report)
    ...
    if element.get('result') is not None:
        extras['result'] = element.get('result')
    if has_bad:
        extras['incomplete'] = True
    return RuleOutput(tag=TAG, text=text, extras=extras)


def _emit_scriptproperty_removed(composite: tuple, element: ElementTree.Element,
                                 report) -> RuleOutput:
    dt_name, pname = composite
    _check_scriptproperty_children(element, composite, report)
    classifications = ['scriptproperty']
    sources_label = render_sources(['core'], None)
    display = f'{dt_name}.{pname}'
    text = format_row(TAG, display, classifications, sources_label, ['REMOVED'])
    extras = {
        'entity_key': ('scriptproperty', composite),
        'kind': 'removed',
        'subsource': 'scriptproperty',
...
def _emit_scriptproperty_modified(composite: tuple, old_el: ElementTree.Element,
                                  new_el: ElementTree.Element,
                                  report) -> Optional[RuleOutput]:
    dt_name, pname = composite
    ...
    sources_label = render_sources(['core'], ['core'])
    display = f'{dt_name}.{pname}'
    text = format_row(TAG, display, classifications, sources_label, changes)
    extras = {
        'entity_key': ('scriptproperty', composite),
        'kind': 'modified',
        'subsource': 'scriptproperty',
...
def _check_scriptproperty_children(element: ElementTree.Element, composite: tuple,
                                   report) -> bool:
    ...
    """
    bad = {c.tag for c in element if c.tag not in _SCRIPTPROPERTY_CHILD_TAGS}
    for tag in sorted(bad):
        report.failures.append((
...
def _diff_scriptproperty_children(old_el: ElementTree.Element,
                                  new_el: ElementTree.Element) -> list[str]:
...
def _collect_all_attrs(element: ElementTree.Element, exclude: tuple[str, ...] = ()
                       ) -> dict[str, str]:
    """Return a copy of `element.attrib` minus the excluded keys."""
    return {k: v for k, v in element.attrib.items() if k not in exclude}


def _index_named_children(element: ElementTree.Element, tag: str
...
def _attr_sig(element: ElementTree.Element) -> tuple:
