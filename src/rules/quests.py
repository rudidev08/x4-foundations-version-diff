...
def _emit(rel: str, kind: ChangeKind,
    ...
    if kind == ChangeKind.ADDED:
        _, extras = render_modified(rel, b'', new or b'', tag=TAG, name=name)
    ...
    if kind == ChangeKind.DELETED:
        _, extras = render_modified(rel, old or b'', b'', tag=TAG, name=name)
    ...
    text, extras = render_modified(
        rel, old or b'', new or b'', tag=TAG, name=name,
    )
    extras.update({
        'entity_key': rel,
        'kind': 'modified',
...
def _classifications(rel: str) -> list[str]:
    ...
    Results are filtered through `_GENERIC_FILTER` for parity with other rules.
    """
    stem = Path(rel).stem
    if stem in _LITERAL_CLASS:
        token = _LITERAL_CLASS[stem]
        return [token] if token not in _GENERIC_FILTER else []
