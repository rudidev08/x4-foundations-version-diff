...
- `<bullet @class>` — labeled `bullet_class`. A change here is a different bullet; the bullet fan-out runs once per shared bullet, so a swap surfaces both the `bullet_class` change here AND zero bullet rows (since the refs differ between sides and don't intersect).
- `<heat @overheat/@coolrate/@cooldelay>` — labels `overheat`, `coolrate`, `cooldelay`.
- `<rotationspeed @max>` — labeled `rotation`.
- `<hull @max>` — labeled `HP`.
...
Fields diffed on the bullet macro (labels are bare — not prefixed with `bullet_`,
unlike the turrets rule):

- `<ammunition @value>` — labeled `damage`.
- `<bullet @speed/@lifetime/@amount/@barrelamount/@timediff/@reload/@heat>` —
  labels `speed`, `lifetime`, `amount`, `barrelamount`, `timediff`, `reload`, `heat`.
