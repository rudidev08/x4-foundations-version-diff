...
def run(old_root: Path, new_root: Path, changes=None) -> list[RuleOutput]:
    ...
    for mid in sorted(new_wares.keys() - old_wares.keys()):
        outputs.extend(_added(new_root, *new_wares[mid], loc_new))
    for mid in sorted(old_wares.keys() - new_wares.keys()):
        outputs.extend(_deleted(old_root, *old_wares[mid], loc_old))
    for mid in sorted(old_wares.keys() & new_wares.keys()):
        outputs.extend(_diff(
            old_root, new_root, mid,
            *old_wares[mid], *new_wares[mid],
            loc_old, loc_new,
        ))
    return outputs


def _load_missile_wares(root: Path) -> dict[str, tuple[str, ET.Element]]:
    ...
    for wares_path in _wares_files(root):
        ...
        tree = ET.parse(wares_path).getroot()
...
def _added(new_root, wares_rel, ware, loc_new) -> list[RuleOutput]:
    ware_id = ware.get('id')
    macro_path = _macro_path(new_root, wares_rel, ware)
    macro = _load_macro(macro_path) if macro_path else None
    name = display_name(macro, loc_new) if macro is not None else ware_id
    source = source_of(wares_rel)
    cls = _missile_class(macro) if macro is not None else None
    ...
    return [RuleOutput(
        tag='missiles',
        text=_format(name, cls, source, parts),
        extras={
            'ware_id': ware_id,
            'name': name,
            'source': source,
...
def _deleted(old_root, wares_rel, ware, loc_old) -> list[RuleOutput]:
    ware_id = ware.get('id')
    macro_path = _macro_path(old_root, wares_rel, ware)
    macro = _load_macro(macro_path) if macro_path else None
    name = display_name(macro, loc_old) if macro is not None else ware_id
    source = source_of(wares_rel)
    cls = _missile_class(macro) if macro is not None else None
    return [RuleOutput(
        tag='missiles',
        text=_format(name, cls, source, ['REMOVED']),
        extras={'ware_id': ware_id, 'name': name, 'source': source, 'class': cls, 'kind': 'removed'},
    )]
...
def _diff(old_root, new_root, ware_id,
    ...
    old_macro = _load_macro(_macro_path(old_root, old_rel, old_ware))
    new_macro = _load_macro(_macro_path(new_root, new_rel, new_ware))
    name = (display_name(new_macro, loc_new) if new_macro is not None
            else display_name(old_macro, loc_old) if old_macro is not None
            else ware_id)
    ...
    for k, (ov, nv) in stat_diff.items():
    ...
    return [RuleOutput(
        tag='missiles',
        text=_format(name, cls_new or cls_old, source, parts),
        extras={
            'ware_id': ware_id,
            'name': name,
            'source': source,
...
def _macro_path(root: Path, wares_rel: str, ware: ET.Element) -> Optional[Path]:
    ...
    # wares.xml lives at `{pkg}/libraries/wares.xml`
    pkg_root = wares_path.parent.parent  # .../libraries → up → pkg root
...
def _load_macro(path: Optional[Path]) -> Optional[ET.Element]:
    if path is None:
        return None
    try:
        return xml_utils.load(path).find('macro')
    except (FileNotFoundError, ET.ParseError):
        return None


def _missile_class(macro: ET.Element) -> Optional[str]:
...
def _stats(macro: ET.Element) -> dict[str, str]:
    out: dict[str, str] = {}
    for xpath, attr, label in _STAT_FIELDS:
        el = macro.find(xpath)
        if el is not None and el.get(attr) is not None:
            out[label] = el.get(attr)
    return out


def _diff_stats(old: ET.Element, new: ET.Element) -> dict[str, tuple[Optional[str], Optional[str]]]:
...
def _format(name: str, cls: Optional[str], source: str, parts: list[str]) -> str:
    cls_label = f' ({cls})' if cls else ''
    source_label = f' [{source}]' if source != 'core' else ''
    return f'[missiles] {name}{cls_label}{source_label}: {", ".join(parts)}'
