...
- `rule` — `libraries/loadoutrules.xml`. The rule's own `@id` isn't stable —
  keys compose applicability attributes instead:
...
# Applicability axes that must NOT be listed as diffed rule attributes.
...
def run(old_root: Path, new_root: Path, changes=None) -> list[RuleOutput]:
    ...
    outputs.extend(_emit_loadout_subsource(
        old_root, new_root, loadout_report, locale_old, locale_new,
    ))

    # --- Rule sub-source ---
    # `key_fn=id(element)` gives each rule a unique key so `_index_by_key`
...
def _emit_loadout_subsource(old_root: Path, new_root: Path, report,
    ...
    for record in report.added:
        outputs.extend(_emit_loadout_added(new_root, record, locale_new))
    for record in report.removed:
        outputs.extend(_emit_loadout_removed(old_root, record, locale_old))
...
def _emit_loadout_added(new_root: Path, record, locale_new: Locale) -> list[RuleOutput]:
    loadout = record.element
    loadout_id = record.key
    macro_ref = loadout.get('macro')
    name = _loadout_display_name(new_root, loadout, locale_new, loadout_id)
    sources_label = render_sources(None, record.sources)
    text = format_row(TAG, name, ['loadout'], sources_label, ['NEW'])
...
def _emit_loadout_removed(old_root: Path, record, locale_old: Locale) -> list[RuleOutput]:
    loadout = record.element
    loadout_id = record.key
    macro_ref = loadout.get('macro')
    name = _loadout_display_name(old_root, loadout, locale_old, loadout_id)
    sources_label = render_sources(record.sources, None)
    text = format_row(TAG, name, ['loadout'], sources_label, ['REMOVED'])
    return [RuleOutput(tag=TAG, text=text, extras={
        'entity_key': ('loadout', loadout_id),
        'kind': 'removed',
        'subsource': 'loadout',
...
def _emit_loadout_modified(old_root: Path, new_root: Path, record,
                           locale_old: Locale, locale_new: Locale) -> list[RuleOutput]:
    loadout_id = record.key
    new_macro = record.new.get('macro')
    name = _loadout_display_name(new_root, record.new, locale_new, loadout_id)
    if name == loadout_id:
        name = _loadout_display_name(old_root, record.old, locale_old, loadout_id)

    changes = _loadout_field_diff(record.old, record.new)
    if not changes:
        return []

    sources_label = render_sources(record.old_sources, record.new_sources)
    text = format_row(TAG, name, ['loadout'], sources_label, changes)
    return [RuleOutput(tag=TAG, text=text, extras={
        'entity_key': ('loadout', loadout_id),
        'kind': 'modified',
        'subsource': 'loadout',
...
def _loadout_display_name(root: Path, loadout: ElementTree.Element,
    ...
    if ident is None:
        return fallback
    resolved = resolve_attr_ref(ident, locale, attribute='name', fallback='')
    return resolved or fallback


def _loadout_field_diff(old_el: ElementTree.Element, new_el: ElementTree.Element) -> list[str]:
    ...
    for slot in ('engine', 'shield', 'turret', 'weapon'):
        old_value = _slot_bag(old_el, slot)
        new_value = _slot_bag(new_el, slot)
        if old_value != new_value:
            added = sorted(_fmt_slot_item(p) for p in (new_value - old_value))
            removed = sorted(_fmt_slot_item(p) for p in (old_value - new_value))
            parts = []
            if added:
    ...
    # Software wares.
    old_value = _software_set(old_el)
    new_value = _software_set(new_el)
    if old_value != new_value:
        added = sorted(new_value - old_value)
        removed = sorted(old_value - new_value)
        parts = []
        if added:
            parts.append('added={' + ','.join(added) + '}')
        if removed:
    ...
    old_value = _virtualmacro_bag(old_el)
    new_value = _virtualmacro_bag(new_el)
    if old_value != new_value:
        added = sorted(new_value - old_value)
        removed = sorted(old_value - new_value)
        parts = []
    ...
    # Ammunition counts.
    old_value = _ammunition_map(old_el)
    new_value = _ammunition_map(new_el)
    if old_value != new_value:
        ammo_parts: list[str] = []
        for key in sorted(set(old_value) | set(new_value)):
            if old_value.get(key) != new_value.get(key):
                ammo_parts.append(f'{key}:{old_value.get(key)}→{new_value.get(key)}')
        if ammo_parts:
...
def _slot_bag(loadout: ElementTree.Element, slot: str) -> frozenset:
    """Return a frozenset of (macro, path) tuples for one equipment slot.

    Each loadout's slot has a per-child `path` attribute that's unique within the
    ...
    if macros_el is None:
        return frozenset()
    items = []
    for index, child in enumerate(macros_el.findall(slot)):
        macro = child.get('macro') or ''
        path = child.get('path') or f'#{index}'
        items.append((macro, path))
    return frozenset(items)


def _software_set(loadout: ElementTree.Element) -> frozenset:
    out = set()
...
def _virtualmacro_bag(loadout: ElementTree.Element) -> frozenset:
    out = set()
...
def _ammunition_map(loadout: ElementTree.Element) -> dict[str, str]:
    ...
    for a in ammo.findall('ammunition'):
        ...
        if not macro:
            continue
        parts = []
        for attribute in ('exact', 'min', 'max'):
            v = a.get(attribute)
            if v is not None:
                parts.append(f'{attribute}={v}')
...
def _emit_rule_subsource(rule_report) -> list[RuleOutput]:
    ...
    The `rule_report` was materialized with unique `id(element)` keys, so we
    ...
    parent_maps: dict[int, dict[int, ElementTree.Element]] = {}
...
def _bucket_rules(tree_root: Optional[ElementTree.Element],
                  parent_maps: dict[int, dict[int, ElementTree.Element]]
                  ) -> dict[tuple, list[ElementTree.Element]]:
    """Group all `<rule>` elements by composite applicability key."""
    buckets: dict[tuple, list[ElementTree.Element]] = {}
...
def _rule_composite_key(rule: ElementTree.Element,
                        pmap: dict[int, ElementTree.Element]) -> Optional[tuple]:
...
def _find_ancestor_tag(element: ElementTree.Element,
                       pmap: dict[int, ElementTree.Element],
                       match_tags: tuple[str, ...]) -> Optional[str]:
    current = element
    seen: set[int] = set()
    while id(current) in pmap and id(current) not in seen:
        seen.add(id(current))
        parent = pmap[id(current)]
...
def _find_ancestor_attr(element: ElementTree.Element,
                        pmap: dict[int, ElementTree.Element],
                        match_tag: str, attribute: str) -> Optional[str]:
    current = element
    seen: set[int] = set()
    while id(current) in pmap and id(current) not in seen:
        seen.add(id(current))
...
def _key_sort_tuple(k: tuple) -> tuple:
    ...
    for element in k:
        if isinstance(element, tuple):
            out.append(tuple(str(x) for x in element))
        else:
...
def _emit_bucket(composite_key: tuple,
                 old_rules: list[ElementTree.Element],
                 new_rules: list[ElementTree.Element]) -> list[RuleOutput]:
...
def _rule_signature(rule: ElementTree.Element) -> tuple:
    """Hashable signature of a rule's diffed attributes (excludes applicability).
...
def _emit_rule_added(composite_key: tuple, rule_el: ElementTree.Element,
    ...
    sources_label = render_sources(None, ['core'])
    parts = ['NEW']
    if sig_parts:
...
def _emit_rule_removed(composite_key: tuple, rule_el: ElementTree.Element,
                       multiset: bool = False) -> RuleOutput:
    name = _rule_display_name(composite_key)
    classifications = _rule_classifications(composite_key)
    sig_parts = _format_rule_attrs(rule_el)
    sources_label = render_sources(['core'], None)
    parts = ['REMOVED']
    if sig_parts:
        parts.append(sig_parts)
    text = format_row(TAG, name, classifications, sources_label, parts)
...
def _emit_rule_modified(composite_key: tuple,
                        old_el: ElementTree.Element, new_el: ElementTree.Element
                        ) -> Optional[RuleOutput]:
    ...
    sources_label = render_sources(['core'], ['core'])
...
def _rule_attr_diff(old_el: ElementTree.Element, new_el: ElementTree.Element) -> list[str]:
    """Label changes between two rules sharing the same applicability key.

    Only diffs the non-applicability attributes.
    """
    out = []
    all_attrs = sorted(set(old_el.attrib) | set(new_el.attrib))
    for attribute in all_attrs:
        if attribute in _RULE_APPLICABILITY_ATTRS:
            continue
        old_value = old_el.get(attribute)
        new_value = new_el.get(attribute)
        if old_value != new_value:
            out.append(f'{attribute} {old_value}→{new_value}')
    return out


def _format_rule_attrs(rule_el: ElementTree.Element) -> str:
    """Compact key=val list of non-applicability attributes for add/remove rows."""
    parts = []
    for attribute in sorted(rule_el.attrib):
        if attribute in _RULE_APPLICABILITY_ATTRS:
            continue
        parts.append(f'{attribute}={rule_el.get(attribute)}')
    return ' '.join(parts)
