...
# Macro stat spec: (xpath_under_macro, attr, label).
MACRO_STATS = [
...
def _dedupe_by_inode(results, old_root: Path, new_root: Path):
    ...
    for rel, kind, old_bytes, new_bytes in results:
        ...
        try:
            ...
            # Stat failed — fall back to rel-path identity.
            key_parts.append(('rel', rel))
...
def _build_row(macro: ET.Element, rel: str, kind: str,
    ...
    text = f'[{TAG}] {macro_name}{cls} {sources_label}: {", ".join(parts)}'
...
def _parse_macro(data: Optional[bytes]) -> Optional[ET.Element]:
    if data is None:
        return None
    try:
...
def _classify(macro: ET.Element) -> list[str]:
...
def _parent_ship_index(side: str, tree_root: Path) -> dict[str, list[str]]:
    ...
    def produce() -> dict[str, list[str]]:
        ...
        for ship_macro_path in _iter_ship_macros(tree_root):
            try:
                doc = ET.parse(ship_macro_path).getroot()
            ...
            for conn in macro.iter('connection'):
                for nested in conn.findall('macro'):
                    ...
                    if ref and ref.startswith('storage_'):
                        idx.setdefault(ref, []).append(ship_name)
